...
class RdfSerializer extends Object {
  ...
  // MODIF LCT: remove the usage of a constant which cannot be redifined
  var $use_advertisement;
	...
	*/
    ...
    // MODIF LCT : initialise with the old mechanism
    $this->use_advertisement = !HIDE_ADVERTISE;

		global $default_prefixes;
    // Fix LCT for the unitary tests
    if ( !empty( $default_prefixes ) ){
	...
	*/
	...
	/**
	* Serializer configuration: Use library advertisement
	* Flag if the serializer should write the advertisement for this library
	* TRUE is default. Default can be changed in constants.php (HIDE_ADVERTISE)
	*
	* @param             boolean
	* @access    public
	*/
	function configUseAdvertisement($bool) {
		$this->use_advertisement = $bool;
	}
	...
	*/
	function writeDescriptions()  {
      ...
      // MODIF LCT: In order to implement the Rdf like it should be! :)
...
//			elseif ($this->canAbbreviateValue($object) &&
        if ($this->canAbbreviateValue($object) &&
			$this->use_attributes &&
