...
Drupal.ckeditorOn = function(textarea_id) {
  ...
  if (teaser) {
    ...
    // [#653498]
    if (teaser.button.attr('value') != Drupal.t('Write a summary')) {
      try {
        teaser.button.click();
      }
      catch (e) {
        teaser.button.val(Drupal.t('Write a summary'));
        teaser.button.attr ('title','Provide a summary description');
      }
    }
    ...
    if (data.length > 1) {
      ...
      if (teaser.button.attr('value') != Drupal.t('Join summary')) {
        try {
          ...
          teaser.button.attr ('title','Join summary');
        }
      }else{
        teaser.button.attr ('title','Join summary');
      }
