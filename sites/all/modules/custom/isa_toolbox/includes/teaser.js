...
Drupal.behaviors.teaser = function(context) {
  ...
  $('textarea.teaser:not(.teaser-processed)', context).each(function() {
    ...
    function join_teaser() {
      ...
      // Change label.
      $(this).val(Drupal.t('Write a summary'));
      $(this).attr ('title','Provide a summary description');
    ...
    if (text.length >= 2) {
      ...
      $('input', button).val(Drupal.t('Write a summary')).toggle(split_teaser, join_teaser);
