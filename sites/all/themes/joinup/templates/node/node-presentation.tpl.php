<?php
// $Id: node-presentation.tpl.php,v 1.4.2.1 2010/05/09 13:37:33 smillart Exp $

/**
 * @file
 *
 * Theme implementation to display a presentation.
 *
 * Available variables:
 ...
 * @see _joinup_preprocess_presentation()
 * @ingroup nodes
 */
?>
			...
			<div class="odd nodes-row-first nodes-row-last clearfix">
				<dl class="colspans-3-5 first last fields">
					<?php foreach ($presentation_info as $vocab => $terms): ?>
						<?php if ($terms): ?>
							<dt class="field field-presentation-info-term"><?php print t($vocab); ?>:</dt>
							<dd class="field field-presentation-info-description"><?php print $terms; ?></dd>
						<?php endif; ?>
					<?php endforeach; ?>
		...
		<?php if (isset($field_presentation_upload_files) && !empty ($field_presentation_upload_files) ): ?>
			<div class="field field-presentationation">
				...
				<ul>
						...
						<?php if ($view['view']): ?>
							<li>
                                    <?php print $additional_doc[$key];  ?>
                                    <?php
                                        $extension = explode(".",$view['filename']);
                                       
                                        if ($extension [count($extension)-1] == "ppt" ||
                                            $extension [count($extension)-1] == "pptx" ||   
                                            $extension [count($extension)-1] == "pps" ||
                                            $extension [count($extension)-1] == "ppsx" ||
                                            $extension [count($extension)-1] == "pdf" ){
                                            print $view['view'];
                                        }
                                  ?>
                                            </li>
