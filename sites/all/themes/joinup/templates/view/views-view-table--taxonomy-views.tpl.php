	...
	<?php foreach ($rows as $count => $row): ?>
		<div class="<?php print implode(' ', $row_classes[$count]); ?> clearfix">
			...
			<?php foreach ($row as $field => $content): ?>
				...
				?>
				<div class="colspan-<?php print $prefix; ($colspans[$col_count][$css_class][$col_number]) ? print $colspans[$col_count][$css_class][$col_number] : print $colspans[$col_count][$col_number]; ?> <?php print $col_class; ?> fields views-field-<?php print $fields[$field]; ?>">
					<?php print $content; ?>
				</div>
				<?php $col_number++; ?>
			<?php endforeach; ?>
