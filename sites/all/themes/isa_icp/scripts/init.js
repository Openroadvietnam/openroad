...
$(document).ready(
	function ()
	{
		...
		$('#filter').find('.collapse-processed').children('a').click( function () { $('#filter-region').toggleClass('collapsed'); return false; } );
		
		// Display/hide the Quick Actions dropdown menu with a sliding motion.
		$('div.buttons').find('a.action.propose-your').click( function () { $('div.quick-actions').children('ul.links').slideToggle(); return false; });
...
//				}
		...
		$('.read-more').click( function () { fnToggleReadMore(this, 'view-footer', 'view-header'); return false; } );
		$('.reduce').click( function () { fnToggleReadMore(this, 'view-header', 'view-footer'); return false; } );
		for (key=0; aViews.lenght; key++)
		{
			$('.' + aViews[key][0]).each( function (index) { var iContentHeight = $(this).height(); if (iContentHeight < aViews[key][1]) $(this).find('.view-footer').hide(); } );
		}
  }
);
 ...
 function advanced_search() {
  ...
  //$('#adsearch').attr("href", hrefval+$('#edit-keys').val());
  var hrefval  = $('#adsearch').attr('href').valueOf() + "/";
