			...
			<div class="odd nodes-row-first nodes-row-last clearfix">
				<dl class="colspans-3-5 first last fields">
					<?php foreach ($document_info as $vocab => $terms): ?>
						...
						<?php endif; ?>
					<?php endforeach; ?>
					<?php if (!empty($original_url['url']) && !empty($original_url['title'])): ?>
						<dt class="field field-original-url-term"><?php print t('Original URL') ?>:</dt>
						<dd class="field field-original-url-description"><a href="<?php print $original_url['url']; ?>"><?php print $original_url['title']; ?></a></dd>
					<?php endif; ?>
