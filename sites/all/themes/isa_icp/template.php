 ...
 * @ingroup phptemplate
 */
function isa_icp_preprocess_page(&$vars) {
  $path = $_GET['q'];
  $args = explode('/', $path);
  ...
  isa_icp_set_active_primary_links($vars);
}

function isa_icp_set_active_primary_links(&$vars) {
  $path = $_GET['q'];
  $path = explode('/', $path);
 ...
 */
function isa_icp_relevant_content_block($nodes, $header=FALSE, $delta = NULL) {
  $output = "";
 ...
 * @ingroup phptemplate
 */
function isa_icp_preprocess_block(&$vars) {
  ...
  if ($block->module == 'menu' && $block->delta == 'menu-news-menu') {
    $block->content = drupal_get_form('isa_icp_menu_news_form');
  }
  // Change block menu news content
  if ($block->module == 'menu' && $block->delta == 'menu-elibrary') {
    $block->content = drupal_get_form('isa_icp_menu_elibrary_form');
  }
 ...
 * @ingroup phptemplate
 */
function isa_icp_preprocess_node(&$vars) {
  $theme = variable_get('theme_default', NULL);
  $theme_path = drupal_get_path('theme', $theme);
  include_once $theme_path . '/includes/template.node.inc';
  _isa_icp_preprocess_node($vars);
}
 ...
 */
function isa_icp_breadcrumb($breadcrumb) {
  global $user;
 ...
 */
function isa_icp_preprocess_content_field(&$variables) {
 ...
 */
function isa_icp_fieldgroup_fieldset($element) {
...
function isa_icp_username($objects, $options = array()) {
...
function isa_icp_menu_elibrary_form($form_state) {
...
function isa_icp_menu_elibrary_form_submit($form, &$form_state) {
  $res = $form_state['values']['list_menu'];
  drupal_goto($res);
}

// Definition of the select form for news menu
function isa_icp_menu_news_form($form_state) {
...
// Manage submit button for news menu 
function isa_icp_menu_news_form_submit($form, &$form_state) {
  $res = $form_state['values']['list_menu'];
  drupal_goto($res);
}
 ...
 */
function isa_icp_preprocess_comment(&$vars) {
...
// set the taxonomy term for users 
  ///isa_icp_set_terms_in_vars($vars, $profile);
 ...
 * @ingroup phptemplate
 */
function isa_icp_preprocess_box(&$vars) {
  ...
  if (module_exists('path')) {
    ...
    if ($node_type == 'topic') {
      ...
      $vars['user_name'] = isa_icp_username($poster);
      $vars['user_company_name'] = $profile->field_company_name['0']['value'];
 ...
 */
function isa_icp_apachesolr_breadcrumb_uid($uid) {
 ...
 * 
 */
function isa_icp_preprocess_simplenews_newsletter_body(&$variables) {
  $content = variable_get('newsletters_mail_body', '...');
  $content = token_replace($content, 'node', $variables['node']);
  $variables['content'] = $content;
}

function isa_icp_preprocess_simplenews_newsletter_footer(&$variables) {
...
function isa_icp_form_element($element, $value) {
...
function isa_icp_comment_submitted($comment) {
...
function isa_icp_node_submitted($node) {
 ...
 * @ingroup themeable
 */
function isa_icp_comment_post_forbidden($node) {
  global $user;
...
function isa_icp_prepare_vote_rating($node) {
...
function isa_icp_prepare_document_attachement($field) {
  global $base_url;
 ...
 *
 */
function isa_icp_user_admin_account($form) {
